...
- [✅] Hardened crash/session recovery for multi-instance runs: replaced the single shared crash-session marker with per-session markers so `ApplicationFlags::NON_UNIQUE` launches no longer generate false `AbnormalExit` reports, added a bounded timeout around secret-portal retrieval to avoid auth/token hangs, and extended Unix clean-shutdown handling beyond SIGINT/SIGTERM.
...
- [✅] Localize absolute dates for older workflow runs so run subtitles use locale-aware date formatting instead of raw ISO `YYYY-MM-DD`.
...
-Recent Updates
- [✅] 2026-05-18 — Prepared the `1.0.14` maintenance release bump without committing: updated release-version targets (`Cargo.toml`, regenerated `Cargo.lock`, `snapcraft.yaml`, `docs/flatpak.md`, `src/demo/logs/job-43021.log`), added a new user-friendly AppStream release entry to `data/metainfo.xml.in`, translated the new bullets for all shipped locales in `po/*.po`, regenerated `po/actioneer.pot` plus compiled `.mo` catalogs, refreshed `flatpak/me.spaceinbox.actioneer.cargo-sources.json`, and rewrote `RELEASE.md` from the changes since tag `1.0.13`. Ran `scripts/extract-translations.sh`, `scripts/compile-translations.sh`, `scripts/regenerate-flatpak-sources.sh`, `scripts/check-flatpak-lock-sync.sh`, `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test -- --skip test_token_storage_lifecycle`, `cargo build`, and `xvfb-run -s "-screen 0 1280x1024x24" cargo test -- --ignored --skip test_token_storage_lifecycle`.
- [✅] 2026-05-18 — Merged Dependabot PR #34 (`ashpd` 0.13.10 → 0.13.11), then ran a full dependency refresh on `develop`: `cargo-outdated` reported the root dependencies were already current, but `cargo update` still advanced several Rust 1.95-compatible transitive packages in `Cargo.lock` (including `aws-lc-rs`/`aws-lc-sys`, `dashmap`, `db-keystore`, `openssl`/`openssl-sys`, the `turso*` family, and `winnow`). Regenerated `flatpak/me.spaceinbox.actioneer.cargo-sources.json`, confirmed lock/Flatpak parity, and revalidated with `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo build`, `cargo test -- --skip test_token_storage_lifecycle`, and `xvfb-run -s "-screen 0 1280x1024x24" cargo test -- --ignored --skip test_token_storage_lifecycle`.
- [✅] 2026-05-18 — Fixed a follow-up CI regression in the Unix shutdown handler: `install_unix_signal_handlers` now enters the existing Tokio runtime before registering `tokio::signal::unix::signal(...)`, which avoids the startup panic seen in the release smoke test (`there is no reactor running`) while preserving the expanded `SIGHUP`/`SIGINT`/`SIGQUIT`/`SIGTERM` cleanup coverage. Rechecked with `cargo build --release`, the targeted shutdown-signal unit test, and `dbus-run-session -- bash tests/smoke/run_smoke.sh`.
- [✅] 2026-05-18 — Fixed the crash/session false-positive path behind issue #33 by refactoring `src/crash_report.rs` to use per-session marker files and only report stale sessions, so concurrent `ApplicationFlags::NON_UNIQUE` launches no longer look like a previous crash; also added targeted unit coverage for stale-vs-live marker handling and marker cleanup when a pending report already exists. In the same pass, bounded `src/storage/secret_portal.rs` secret retrieval with an explicit Tokio timeout plus unit tests for fast/slow portal requests, and expanded Unix clean-shutdown coverage in `src/main.rs` from `SIGINT`/`SIGTERM` to `SIGHUP`/`SIGQUIT`/`SIGTERM` via a shared `ShutdownSignal` model with coverage for the registered signal set. Revalidated with `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo build`, `cargo test -- --skip test_token_storage_lifecycle`, and `xvfb-run -s "-screen 0 1280x1024x24" cargo test -- --ignored --skip test_token_storage_lifecycle`.
- [✅] 2026-05-12 — Bumped Actioneer to `1.0.13` across release metadata and packaging files (`Cargo.toml`, `Cargo.lock`, `snapcraft.yaml`, `docs/flatpak.md`, `src/demo/logs/job-43021.log`), added a new maintenance AppStream release entry to `data/metainfo.xml.in` for localized old-run dates and Linux build reliability, translated the new bullets for all shipped locales in `po/*.po`, regenerated `po/actioneer.pot` plus compiled catalogs with `scripts/extract-translations.sh` and `scripts/compile-translations.sh`, refreshed `RELEASE.md`, and revalidated with `cargo generate-lockfile`, `scripts/regenerate-flatpak-sources.sh`, `scripts/check-flatpak-lock-sync.sh`, `cargo fmt --all`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test`, `cargo build`, and `xvfb-run -s "-screen 0 1280x1024x24" cargo test -- --ignored`.
- [✅] 2026-05-12 — Switched `gettextrs` to the `gettext-system` feature in `Cargo.toml`, so Linux builds link against the platform gettext/libintl instead of compiling vendored GNU gettext 0.26; this avoids the known C23/gnulib `_Generic` failure in `gettext-sys` while keeping i18n behavior intact. Revalidated with `cargo fmt --all -- --check`, `cargo check`, `cargo build`, `cargo test`, and `cargo clippy --all-targets --all-features -- -D warnings`.
- [✅] 2026-05-12 — Localized the absolute-date fallback for older workflow runs in `src/api/models.rs`: runs newer than a week still use relative strings, but older ones now format dates through the active app locale (via GLib `%x`) with a deterministic per-language numeric fallback instead of always showing raw ISO `YYYY-MM-DD`; also added unit coverage for the new fallback/date-order behavior.
