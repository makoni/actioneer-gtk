...
- [🔄] Break `src/ui/main_window.rs` (~1.3K LOC) into dedicated modules (app state, repo list pane, async loaders) to unblock further readability improvements. (Sidebar panel extracted into `ui/main_window/sidebar_panel.rs`; next up: async loader/state modules.)
