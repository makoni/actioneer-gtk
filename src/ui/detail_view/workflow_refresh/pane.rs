//! The detail pane's refresh behaviour.
//!
//! Split out of `workflow_refresh.rs`: this impl block was 525 of its 971
//! lines. What stays next door is the policy the refresh follows — when to
//! skip, when to reconfigure the timer — which is pure and separately tested.
...
impl RepoDetailPane {
            ...
            match result {
                Ok(wf_list) => {
                    ...
                    *workflows.lock() = wf_list.clone();
                    ...
                    crate::ui::detail_view::workflow_list::update_workflows_list(
                        &ui_context,
                        wf_list.as_ref(),
                    );
                }
    ...
    pub(super) fn refresh_runs_background(context: &WorkflowListContext) {
        let client = context.client.clone();
        let owner = context.owner.clone();
        ...
        for row in crate::ui::detail_view::workflow_list::collect_workflow_rows(&context.store) {
    ...
    pub(super) fn clone_for_callbacks(&self) -> CallbackRefs {
        CallbackRefs {
