//! The "Trigger workflow" dialog.
//!
//! Split out of `workflows.rs`, where it was ~550 lines inside a single
//! 1055-line row builder. Everything it closes over was already prepared as
//! separate locals at the call site, so the move is the closure verbatim plus a
//! struct to carry the captures.
...
pub(super) struct TriggerContext {
    ...
    pub workflow_display: String,
    pub parent_window: adw::ApplicationWindow,
    pub toast_overlay: adw::ToastOverlay,
    pub expander: gtk::glib::WeakRef<gtk::Expander>,
    ...
    pub workflows_with_active_shared: Arc<Mutex<HashSet<i64>>>,
    pub workflows_loading_shared: Arc<Mutex<HashSet<i64>>>,
    pub workflows_last_loaded: Arc<Mutex<std::collections::HashMap<i64, std::time::Instant>>>,
    pub workflow_path: String,
    pub run_list: WorkflowRunListModel,
}

pub(super) fn connect_trigger_button(trigger_btn: &gtk::Button, ctx: TriggerContext) {
    let TriggerContext {
        ...
        run_filters: run_filters_for_trigger,
        run_load_service: run_load_service_for_trigger,
        workflows_with_active_shared,
        workflows_loading_shared,
